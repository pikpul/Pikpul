from Pikpulbot.modules.sql_helper import warns_sql as sql
from Pikpulbot.utils import friday_on_cmd
from Pikpulbot import CMD_HELP

@Pukpul.on(Pikpul_on_cmd(pattern="warn(?: |$)(.*)"))
async def _s(event):
...
@Pikpul.on(Pikpul_on_cmd(pattern="rwarn(?: |$)(.*)"))
async def _(event):
    user, reason = await get_user_from_event(event)
    sed = await friday.get_permissions(event.chat_id, user.id)
    if sed.is_admin:
        await event.edit("Demn, Admins Can't Be Warned")
        return
    dragon = await Pikpul.get_permissions(event.chat_id, bot.uid)
    if not dragon.is_admin:
        await event.edit("Demn, Me nOT Admin")
        return
    sql.reset_warns(user.id, event.chat_id)
...
@Pikpul.on(Pikpul_on_cmd(pattern="allwarns(?: |$)(.*)"))
async def __(event):
    user, reason = await get_user_from_event(event)
...
@Pikpul.on(Pikpul_on_cmd(pattern="slimit ?(.*)"))
async def m_(event):
...
@Pikpul.on(Pikpul_on_cmd(pattern="wap ?(.*)"))
async def m_(event):
    args = event.pattern_match.group(1)
    if args:
