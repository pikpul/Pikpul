...
from Pikpulbot import CMD_HELP, LOGS, TEMP_DOWNLOAD_DIRECTORY
from Pikpulbot.events import register
from fridaybot.utils import edit_or_reply, Pikpul_on_cmd, sudo_cmd
...
@Pikpul.on(pikpul_on_cmd(pattern="download(?: |$)(.*)", outgoing=True))
@Pikpul.on(sudo_cmd(pattern="download(?: |$)(.*)", allow_sudo=True))
async def download(target_file):
    """ For .dl command, download files to the Pikpulbot's server. """
    Pikpul = await edit_or_reply(target_file, "`Processing ...`")
    await Pikpul.edit("Processing using fridaybot server ( ◜‿◝ )♡")
        ...
        while not downloader.isFinished():
            ...
            try:
                ...
                if round(diff % 10.00) == 0 and current_message != display_message:
                    await Pikpul.edit(current_message)
...
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the pikpulbot's server """
    await u_event.edit("Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("Pikpulbot.session", "config.env"):
