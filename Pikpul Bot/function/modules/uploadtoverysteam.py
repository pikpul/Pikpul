...
import magic
import requests
from uniborg.util import Pikpul_on_cmd, progress

from Pikpulbot import CMD_HELP


@Pikpul.on(Pikpul_on_cmd(pattern="verystream ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
CMD_HELP.update(
    {
        "uploadtoverystream": "**Upload to very stream**\
\n\n**Syntax : **`.verystream <reply to file>`\
\n**Usage :** uploades the file to very stream."
    }
