...
@apikpul.on(Pikpul_on_cmd(pattern="wshot ?(.*)"))
@Pikpul.on(sudo_cmd(pattern="wshot ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    driver.get(urlissed)
    driver.get_screenshot_as_file("Webshot-@Pikpulot.png")
    imgpath = "Webshot-@Pikpulot.png"
...
@Pikpul.on(Pikpul_on_cmd(pattern="lp ?(.*)"))
@Pikpul.on(sudo_cmd(pattern="lp ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    try:
...
@Pikpul.on(Pikpul_on_cmd(pattern="bin ?(.*)"))
@Pikp.on(sudo_cmd(pattern="bin ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    try:
...
@Pikpul.on(Pikpul_on_cmd(pattern="iban ?(.*)"))
@Pikpul.on(sudo_cmd(pattern="iban ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@Pikpul.on(Pikpul_on_cmd(pattern="gitdl ?(.*)"))
@Pikpul.on(sudo_cmd(pattern="gitdl ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    try:
...
@Pikpul.on(friday_on_cmd(pattern="yts ?(.*)"))
@Pikpul.on(sudo_cmd(pattern="yts ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    try:
...
@Pikpul.on(Pikpul_on_cmd(pattern="akd ?(.*)"))
@Pikpul.on(sudo_cmd(pattern="akd ?(.*)", allow_sudo=True))
async def _(event):
    akkad = event.pattern_match.group(1)
    if event.fwd_from:
        return
    pathz, name = await apk_dl(akkad, Config.TMP_DOWNLOAD_DIRECTORY, event)
    await borg.send_file(event.chat_id, pathz, caption='Uploaded By @PikpulOT')
