...
@Pikpul.on(Pikpul_on_cmd(pattern="search (.*)"))
@Pikpul.on(sudo_cmd(pattern="search (.*)", allow_sudo=True))
async def _(event):
...
@Pikpul.on(Pikpul_on_cmd(pattern="image (.*)"))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
@Pikpul.on(Pikpul_on_cmd(pattern="grs"))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
