...
from Pikpulbot.utils import edit_or_reply, friday_on_cmd, sudo_cmd


@Pikpul.on(friday_on_cmd(pattern="ytmusic ?(.*)"))
@Pikpul.on(sudo_cmd(pattern="ytmusic ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
